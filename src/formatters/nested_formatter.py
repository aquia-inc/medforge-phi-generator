...
class NestedEmailFormatter(BaseEmailFormatter):
    ...
    def _read_file_as_attachment(self, filepath):
        """Read a file and return (data, filename, subtype) tuple for _build_and_save_email."""
        ...
        with open(filepath, 'rb') as f:
            return (f.read(), os.path.basename(filepath), subtype)
...
"""
        ...
        if os.path.exists(lab_pdf_path):
            attachments.append(self._read_file_as_attachment(lab_pdf_path))
...
"""
        attachments = [
            self._read_file_as_attachment(p) for p in attachment_paths if os.path.exists(p)
        ]

        return self._build_and_save_email(
            subject=f"Medical Records - {patient['last_name']}, {patient['first_name']}",
            from_addr="Medical Records <records@healthsystem.org>",
            to_addr=f"{provider['first_name']} {provider['last_name']} <{provider['email']}>",
            plain_body=body_text,
            attachments=attachments or None,
            filename=filename,
        )
...
"""
        ...
        if os.path.exists(progress_note_path):
            attachments.append(self._read_file_as_attachment(progress_note_path))

        return self._build_and_save_email(
            subject=f"Patient Referral: {patient['last_name']}, {patient['first_name']}",
            from_addr=f"{referring_provider['first_name']} {referring_provider['last_name']} <{referring_provider['email']}>",
            to_addr=f"{specialist_provider['first_name']} {specialist_provider['last_name']} <{specialist_provider['email']}>",
            plain_body=body_text,
            attachments=attachments or None,
            filename=filename,
        )
...
"""
        ...
        if os.path.exists(form_path):
            attachments.append(self._read_file_as_attachment(form_path))

        return self._build_and_save_email(
            subject="Updated Patient Registration Forms",
            from_addr=f"Office Manager <manager@{facility['name'].lower().replace(' ', '')}.org>",
            to_addr=f"Front Desk Staff <frontdesk@{facility['name'].lower().replace(' ', '')}.org>",
            plain_body=body_text,
            attachments=attachments or None,
            filename=filename,
        )
...
"""
        ...
        if os.path.exists(policy_pdf_path):
            attachments.append(self._read_file_as_attachment(policy_pdf_path))

        return self._build_and_save_email(
            subject="New Clinical Documentation Policy - Action Required",
            from_addr=f"Compliance Department <compliance@{facility['name'].lower().replace(' ', '')}.org>",
            to_addr=f"All Clinical Staff <clinical@{facility['name'].lower().replace(' ', '')}.org>",
            plain_body=body_text,
            attachments=attachments or None,
            filename=filename,
        )
    ...
    def _attach_file(self, msg, filepath):
        """Legacy helper — delegates to BaseEmailFormatter._attach_file_from_path."""
        self._attach_file_from_path(msg, filepath)
